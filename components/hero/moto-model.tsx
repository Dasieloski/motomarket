...
// Nota: la compresión real (DRACO / Meshopt, texturas KTX2) debe hacerse en el asset.
// Aquí solo configuramos el loader para aprovechar esas optimizaciones si existen.
...
export function MotoModel({ scrollProgress }: MotoModelProps) {
  ...
  // Marcar modelo listo cuando el clone existe (así el grupo ya se renderiza y useFrame puede actuar)
  useEffect(() => {
    if (clonedScene) {
  ...
  // Aplicar opacity a todos los materiales para fade-in suave
  useFrame(() => {
    if (clonedScene && opacity < 1) {
